...
class Multiball(game.Mode):
    ...
    def ball_drained(self):
        #print("ball_drained locklit false")
        #self.lock_lit = False
        self.update_lamps()
...
#                   self.layer = dmd.GroupedLayer(128, 32, [self.animation_layer,self.text_layer])
    ...
    def eject_lock(self, num=1):
        left_to_eject=num-1
        self.game.coils.lockupEject.pulse(20)
        wait=0.125
        while left_to_eject > 0:
            self.delayed_name = self.delay(delay=wait, handler=self.eject_lock)
            left_to_eject -= 1
            wait += 0.125        
